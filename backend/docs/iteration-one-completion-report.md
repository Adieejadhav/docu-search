...
```powershell
cd C:\docu-search\backend
.\.venv\Scripts\python.exe -m scripts.ingest_documents "C:\Users\ADINATH\Downloads\rag_robust_format_test_corpus\rag_robust_format_test_corpus" --recursive --normalized-output-json "..\storage\rag_robust_format_test_corpus.normalized.json" --chunks-output-json "..\storage\rag_robust_format_test_corpus.chunks.json"
```
...
```powershell
cd C:\docu-search\backend
.\.venv\Scripts\python.exe -m scripts.ask_index "Which policy mentions the 14-day satellite-mode exception?" --top-k 5 --show-context
```
...
```powershell
cd C:\docu-search\backend
.\.venv\Scripts\python.exe -m scripts.clear_database
```
